...
export default function ServiceCard({ servicio, onAgendar }) {
  ...
  return (
    <article className="group relative bg-white dark:bg-[#0B1221] border border-[#EAEAEA] dark:border-slate-800/60 rounded-xl overflow-hidden card-lift flex flex-col">
      ...
      <div className="relative aspect-card overflow-hidden bg-gradient-to-br from-slate-100 to-slate-50 dark:from-slate-800/30 dark:to-[#03070e]">
        ...
        <div className="absolute top-4 left-4 px-3 py-1.5 rounded-full bg-white/90 dark:bg-[#0B1221]/90 backdrop-blur-sm border border-white/40 dark:border-slate-700/30 shadow-none">
          <span className="text-[10px] font-bold text-[#2F3437] dark:text-slate-300 uppercase tracking-widest tabular">
      ...
      <div className="p-4 flex flex-col flex-1">
        <h3 className="font-display text-xl font-semibold text-[#111111] dark:text-white leading-tight mb-2">
          {servicio.nombre}
        </h3>

        {servicio.descripcion && (
          <p className="text-sm text-[#787774] dark:text-slate-400 leading-relaxed line-clamp-2 mb-3">
            {servicio.descripcion}
          </p>
        )}
        ...
        <div className="mt-auto pt-5 border-t border-black/5 dark:border-slate-800/40 flex items-center justify-between gap-3">
          <div>
            <p className="text-[10px] uppercase tracking-widest text-[#A8A29E] font-bold mb-0.5">Desde</p>
            <p className="font-display text-xl font-bold text-[#111111] dark:text-white tabular">{precioFmt}</p>
          </div>
