...
export default function ReviewModal({ cita, onClose, onReviewSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-card border border-line dark:border-slate-800 rounded-xl p-6 w-full max-w-md shadow-2xl">
        <h2 className="text-2xl font-bold text-ink dark:text-white mb-2 text-center">
          Califica tu servicio
        </h2>
        <p className="text-muted dark:text-slate-400 text-center text-sm mb-6">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {/* CAJA DE COMENTARIOS */}
          <div>
            <label className="block text-sm font-medium text-ink-2 dark:text-slate-300 mb-1">
              Comentario (Opcional)
            </label>
            <textarea
              rows="3"
              placeholder="Ej: Excelente servicio, muy puntual..."
              className="w-full px-4 py-3 rounded-xl bg-paper-2 dark:bg-slate-900 border border-line dark:border-slate-800 focus:ring-2 focus:ring-emerald-500 outline-none resize-none text-ink dark:text-white"
              value={comentario}
          ...
          <div className="flex gap-3 mt-4">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-3 px-4 bg-paper hover:bg-line dark:bg-slate-800 dark:hover:bg-slate-700 text-ink-2 dark:text-slate-300 rounded-xl font-bold transition-colors"
            >
              Cancelar
            </button>
            <button
